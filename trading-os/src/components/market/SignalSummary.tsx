...
import { TREND_LABEL, SETUP_LABEL, SETUP_STAGE_LABEL } from "@/lib/labels";
  ...
  return (
    <Card className="flex h-full flex-col">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-text">信号</h3>
      ...
      <div className="space-y-3">
        <Row label="趋势" value={TREND_LABEL[signal.trend]} tone={signal.trend === "BULL" ? "text-bull" : signal.trend === "BEAR" ? "text-bear" : "text-warn"} />
        <Row label="结构" value={signal.trend === "BULL" ? "高点抬高 → 低点抬高" : signal.trend === "BEAR" ? "低点降低 → 高点降低" : "区间震荡"} />
        <Row label="形态" value={`${SETUP_LABEL[signal.setup]} → ${SETUP_STAGE_LABEL[signal.setupStage]}`} />
        <div className="border-t border-border pt-3">
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted">信号评分</span>
