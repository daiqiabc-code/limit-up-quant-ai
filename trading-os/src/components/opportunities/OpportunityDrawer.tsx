...
import { ACTION_LABEL, actionTone, REGIME_LABEL, regimeTone, riskTone, RISK_LABEL, TREND_LABEL } from "@/lib/labels";
      ...
      {opp && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Market State" value={<Badge tone={regimeTone(opp.regime.state)}>{REGIME_LABEL[opp.regime.state]}</Badge>} />
            <Field label="Trend" value={TREND_LABEL[opp.trend]} />
            <Field label="Signal Score" value={<span className="text-lg font-bold text-text">{opp.score}</span>} />
            <Field label="Setup" value={opp.setup} />
          </div>

          <div className="rounded-lg border border-border bg-surface p-3">
            <div className="mb-2 grid grid-cols-3 gap-2 text-center">
              <div>
                <div className="text-[10px] uppercase text-muted">Entry</div>
                <div className="num text-sm font-semibold text-text">{formatPrice(opp.entry)}</div>
              </div>
              <div>
                <div className="text-[10px] uppercase text-muted">Stop</div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-center text-xs">
              <Field label="R:R" value={opp.riskReward.toFixed(1)} />
              <Field
                label="Risk"
          ...
          <div>
            <div className="mb-1 text-[11px] uppercase text-muted">Reason</div>
